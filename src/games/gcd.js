...
export const getUniqGame = () => {
    let randomNumber1 = getRandomInt();
    let randomNumber2 = getRandomInt();
    const question = `${randomNumber1} ${randomNumber2}`;

    //цикл продолжается, пока числа не станут равны нулю
    while (randomNumber1 !== 0 && randomNumber2 !== 0){
        if (randomNumber1 > randomNumber2) { //если первое больше второго, то первому присваиваем остаток от деления первого на второе
